...
#   ✅ Whisper robusto no Windows: NÃO assume nome do mp3; busca o arquivo gerado
...
#   ffmpeg instalado no sistema (ou FFMPEG_PATH no .env)
# -------------------------------------------------------------------------------------------------------------
...
defaults = {
    ...
    # <<< GOVERNANÇA DE IA (NOVO): só consome token sob demanda
...
def _transcrever_por_whisper(url: str) -> str:
    """
    Versão robusta no Windows:
    - Não assume 'audio.mp3'
    - Faz download e pós-processamento
    - Procura o MP3 realmente gerado na pasta temporária
    """
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        ydl_opts = {
            ...
            "quiet": True,
            "noplaylist": True,
            "ffmpeg_location": os.getenv("FFMPEG_PATH"),  # opcional (recomendado)
            "postprocessors": [
                {
        ...
        if not audio_file or not os.path.exists(audio_file):
            raise FileNotFoundError(
                "Arquivo MP3 não foi gerado pelo yt-dlp. Verifique FFmpeg/ffprobe e permissões."
            )
    ...
    if arquivo:
        with st.spinner("🔄 Rodando ETL ILUMEO..."):
            ...
            try:
                ...
                # <<< GOVERNANÇA DE IA (NOVO): resetar outputs/autorizações quando um novo ETL roda
                st.session_state["autorizar_insights"] = False
                st.session_state["insights_gerados"] = False
                st.session_state["autorizar_conteudos"] = False
                st.session_state["insights"] = ""
                st.session_state["conteudos_multicanais"] = ""
        ...
        # ---------------------------------------------------------------------
        # IA — INSIGHTS (SOMENTE COM AUTORIZAÇÃO EXPLÍCITA)
        # ---------------------------------------------------------------------
        st.markdown("---")
        st.subheader("Insight Profundo da Pesquisa")
        st.info("⚠️ A análise avançada com IA só será executada mediante autorização")

        # <<< GOVERNANÇA DE IA (NOVO): botão de autorização
        ...
        # <<< GOVERNANÇA DE IA (NOVO): executa apenas sob demanda e uma única vez
        ...
        # ---------------------------------------------------------------------
        # IA — CONTEÚDOS MULTICANAIS (SOMENTE COM AUTORIZAÇÃO EXPLÍCITA)
        # ---------------------------------------------------------------------
        st.subheader("Conteúdo Multicanal Gerado Automaticamente")

        st.info("⚠️ A geração de conteúdos multicanais com IA só será executada mediante autorização. Clique aqui!")

        # <<< GOVERNANÇA DE IA (NOVO): só habilita o botão de conteúdo após insights
